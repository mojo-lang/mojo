...
*标识符(identifier)* 可以由以下的字符开始：小写的字母 `a` 到 `z`、基本多语言平面 (Basic Multilingual Plane) 中非字符数字组合的 Unicode 字符以及基本多文种平面以外的非个人专用区字符。在首字符之后，允许使用数字、下划线`_`和组合 Unicode 字符。
...
下面这些被保留的关键字除了`attribute`、`package`、`service`、`type`之外不允许用作标识符，具体描述请参考 [标识符](#identifiers)。

* 用在声明中的关键字：`attribute`、`const`、`enum`、`func`、`import`、`package`、`service`、`type` 以及 `var`。
* 用在语句中的关键字：`break`、`continue`、`else`、`for`、`if`、`in`、`match`、`repeat`、`return` 以及 `while`。
* 用在表达式中的关键字：`false`、`null`、`true`。
* 用在操作符中的关键字：`and`、`or`、`not`、`xor`。
* 用在类型中的关键字：`as`、`is`。
* 用在模式中的关键字：`_`。

| Declarations | Statements | Expressions | Operators | Patterns | Types |
| ------------ | ---------- | ----------- | --------- | -------- | ----- |
| `const`      | `break`    | `null`      | `and`     | `_`      | `as`  |
| `enum`       | `continue` | `true`      | `or`      |          | `is`  |
| `func`       | `else`     | `false`     | `not`     |          |       |
| `import`     | `for`      |             | `xor`     |          |       |
| `service`    | `if`       |             | `inv`     |          |       |
| `type`       | `in`       |             |           |          |       |
