...
*运算符*是检查、改变、合并值的特殊符号或短语。例如，加号（`+`）将两个数相加（如 `var i = 1 + 2`）。更复杂的运算例子包括逻辑与运算符 `&&`（如 `if enteredDoorCode && passedRetinaScan`）。

Mojo 所支持运算符你可能在别的语言比如 C 语言里已经认识了，同时为了减少常见编码错误对它们做了部分改进。如：赋值符（`=`）不再有返回值，这样就消除了手误将判等运算符（`==`）写成赋值符导致代码错误的缺陷。算术运算符（`+`，`-`，`*`，`/`，`%` 等）的结果会被检测并禁止值溢出，以此来避免保存变量时由于变量大于或小于其类型所能承载的范围时导致的异常结果。

Mojo 还提供了 C 语言没有的区间运算符，例如 `a..<b` 或 `a..b`，这方便我们表达一个区间内的数值。

本章节只描述了 Mojo 中的基本运算符，[高级运算符](./27_Advanced_Operators.md) 这章会包含 Mojo 中的高级运算符，及如何自定义运算符，及如何进行自定义类型的运算符重载。
...
- *三元*运算符操作三个操作对象，和 C 语言一样，Mojo 只有一个三元运算符，就是三目运算符（`a ? b : c`）。
...
与 C 语言和 Objective-C 不同，Mojo 的赋值操作并不返回任何值。所以下面语句是无效的：
...
通过将 `if x = y` 标记为无效语句，Mojo 能帮你避免把 （`==`）错写成（`=`）这类错误的出现。

## 算术运算符 {#arithmetic-operators}

Mojo 中所有数值类型都支持了基本的四则*算术运算符*：
...
与 C 语言和 Objective-C 不同的是，Mojo 默认情况下不允许在数值运算中出现溢出情况。但是你可以使用 Mojo 的溢出运算符来实现溢出运算（如 `a &+ b`）。详情参见 [溢出运算符](./27_Advanced_Operators.md#overflow-operators)。
...
如同 C 语言，Mojo 也提供把其他运算符和赋值运算（`=`）组合的*组合赋值运算符*，组合加运算（`+=`）是其中一个例子：
...
更多 Mojo 标准库运算符的信息，请看 [运算符声明](https://developer.apple.com/documentation/swift/operator_declarations)。
‌
...
> 注意
> 
> Mojo 也提供恒等（`===`）和不恒等（`!==`）这两个比较符来判断两个对象是否引用同一个对象实例。更多细节在 [类与结构](./09_Structures_And_Classes.md) 章节的 **Identity Operators** 部分。
...
> 注意
> 
> Mojo 标准库只能比较七个以内元素的元组比较函数。如果你的元组元素超过七个时，你需要自己实现比较运算符。
...
*逻辑运算符*的操作对象是逻辑布尔值。Mojo 支持基于 C 语言的三个标准逻辑运算。
...
> 注意
> 
> Mojo 逻辑操作符 `&&` 和 `||` 是左结合的，这意味着拥有多元逻辑操作符的复合表达式优先计算最左边的子表达式。
