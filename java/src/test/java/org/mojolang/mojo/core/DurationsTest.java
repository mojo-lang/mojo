...
import static org.mojolang.mojo.core.Durations.toSecondsAsDouble;
...
public class DurationsTest {
    ...
    private static final Duration INVALID_MAX =
            Duration.newBuilder().setSeconds(Long.MAX_VALUE).setNanoseconds(Integer.MAX_VALUE).build();
    private static final Duration INVALID_MIN =
            Duration.newBuilder().setSeconds(Long.MIN_VALUE).setNanoseconds(Integer.MIN_VALUE).build();
    ...
    @Test
    public void testDurationStringFormat() throws ErrorException, ParseException {
        ...
        duration = Duration.newBuilder().setNanoseconds(10000000).build();
        assertThat(Durations.toString(duration)).isEqualTo("0.010s");
        duration = Duration.newBuilder().setNanoseconds(10000).build();
        assertThat(Durations.toString(duration)).isEqualTo("0.000010s");
        duration = Duration.newBuilder().setNanoseconds(10).build();
        assertThat(Durations.toString(duration)).isEqualTo("0.000000010s");

        // Parsing accepts an fractional digits as long as they fit into nano
        // precision.
        duration = Durations.parse("0.1s");
        assertThat(duration.getNanoseconds()).isEqualTo(100000000);
        duration = Durations.parse("0.0001s");
        assertThat(duration.getNanoseconds()).isEqualTo(100000);
        duration = Durations.parse("0.0000001s");
        assertThat(duration.getNanoseconds()).isEqualTo(100);
        ...
        assertThat(duration.getNanoseconds()).isEqualTo(999999999);
        duration = Durations.parse("-315576000000.999999999s");
        assertThat(duration.getSeconds()).isEqualTo(-315576000000L);
        assertThat(duration.getNanoseconds()).isEqualTo(-999999999);
        // Repeat using parseUnchecked().
    ...
    public void testDurationInvalidFormat() {
        ...
        // Invalid nanos value.
        try {
            Durations.toString(Duration.newBuilder().setSeconds(1).setNanoseconds(-1).build());
            fail();
        } catch (IllegalArgumentException expected) {
            assertThat(expected).hasMessageThat().isNotNull();
        }
    ...
    @Test
    public void testDurationConversion() throws ErrorException, ParseException {
    ...
    @Test
    public void testOverflows() throws ErrorException {
        try {
    ...
    static Duration duration(long seconds, int nanos) {
        return Durations.checkValid(
                Durations.checkValid(Duration.newBuilder().setSeconds(seconds).setNanoseconds(nanos)));
    }
}
