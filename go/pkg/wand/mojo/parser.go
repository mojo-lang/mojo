...
func (p *Parser) parseWandFile(path string) (*lang.Package, error) {
	parser := parser.New()
	wandFile := path2.Join(path, "wand.mojo")
	file, err := parser.ParseFile(wandFile)
	if err != nil {
		logs.Errorw("failed to parse wand file", "file", wandFile, "error", err.Error())
		return nil, err
	}

	if len(file.Statements) == 0 {
		logs.Errorw("not a valid wand.mojo file, has no statement include", "file", wandFile)
		return nil, errors.New("there is no package declaration in wand file")
	}

	decl := file.Statements[0].GetDeclaration()
	if decl == nil {
		logs.Errorw("not a valid wand.mojo file, has no declaration statement include", "file", wandFile)
		return nil, errors.New("there is no package declaration in wand file")
	}
