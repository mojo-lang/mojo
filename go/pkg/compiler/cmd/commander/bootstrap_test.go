...
func TestBootstrapLocalSourcesUnified(t *testing.T) {
	for _, command := range []string{"protoc", "protoc-gen-go", "protoc-gen-go-grpc"} {
		if _, err := exec.LookPath(command); err != nil {
			t.Skipf("integration test requires %s", command)
		}
	...
	for _, name := range mpm.MojoPackageNames {
		...
		if name == "core" {
			...
			dependencies = "dependencies: { 'mojo.core': {repository: 'github.com/mojo-lang/mojo/mojo/core'} }"
		}
		manifest = fmt.Sprintf("package mojo.%s {\nrepository: 'github.com/mojo-lang/mojo'\n%s\n}", name, dependencies) + "\n" + manifest
		write("mojo/"+name+"/bootstrap.mojo", source)
	}
	...
	// generated intermediate protobuf, rather than a stale file in that output.
	write("mojo/core/bootstrap.mojo", "type String\ntype BootstrapProbe { fresh: String @1 }")
	b := Builder{Pwd: root, Path: filepath.Join(root, "mojo/core"), Targets: "go", Output: "custom-go"}
	require.NoError(t, b.Execute())
	require.Contains(t, read("custom-go/pkg/mojo/core/bootstrap.pb.go"), "GetFresh()")
	require.Contains(t, read("protobuf/mojo/core/bootstrap.proto"), "string fresh = 1;")
	require.NoFileExists(t, filepath.Join(root, "custom-go/go.mod"))
}
