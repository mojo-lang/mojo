...
func (v *ProtoVisitor) VisitTopLevelDef(ctx *TopLevelDefContext) interface{} {
	...
	if extendDef := ctx.ExtendDef(); extendDef != nil {
		if decl, ok := extendDef.Accept(NewExtendDefVisitor()).(*lang.StructDecl); ok {
			decl.Attributes = append(decl.Attributes, lang.NewBoolAttribute("protobuf", "extend"))
			return lang.NewStructDeclaration(decl)
		}
	}
