...
func TestLookupIdentifier(t *testing.T) {
    ctx := WithScope(Empty())
    Scope(ctx).Declare(lang.NewTypeAliasDeclaration(&lang.TypeAliasDecl{Name: "Test", PackageName: "test"}))

    ctx2 := WithScope(ctx)
    Scope(ctx2).Declare(lang.NewTypeAliasDeclaration(&lang.TypeAliasDecl{Name: "Test2", PackageName: "test"}))
